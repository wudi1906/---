...
### 痛点→解决方案速览
- 痛点：买家需要一次性看到跨领域交付能力，却担心 Demo 启动复杂。
- 解决方案：本指南 + START_ALL/STOP_ALL/TEST_ALL + 门户 `http://localhost:8101`，10 分钟完成全套演示。
- 交付物：六个项目源代码/脚本、Postman 集合、演示脚本、SLA 模板、Upwork/Fiverr 文案。
- SLA：建议在 Basic/Standard/Premium 套餐中承诺 7/14/30 天远程支持，<1h 响应。
...
**Postman 集合**：`doc-knowledge-forge/postman/doc_knowledge_forge.postman_collection.json`

**提示**：首次启动会下载嵌入模型（约 200 MB），请保持联网；模型和上传文件会缓存到 `app/.cache`、`uploads/` 与 SQLite 数据库，可按需清理。

**测试上传**：
创建测试文件后通过主页上传，或使用 API 文档的 `/api/docs/upload` 测试
...
> 💡 **流程建议（Process）**：启动 → 门户概览 → 单项目深潜 → 回到门户强调 CTA 与 SLA，最后引导 Upwork/Fiverr 下单。
...
## ❓ FAQ & 支持
- **Q:** 如何处理买家定制需求？
  **A:** 可在 Upwork/Fiverr 套餐中说明可选模块（KPI、自定义集成、部署服务）。
- **Q:** 售后支持范围？
  **A:** 推荐 Basic/Standard/Premium 提供 7/14/30 天远程支持，含 bug 修复与部署指导。
- **Q:** 如何预约演示？
  **A:** 邮件 you@example.com 或通过 Upwork/Fiverr 发送需求。

## 📞 CTA · 下一步
- 🔵 [Upwork · 立即咨询](https://www.upwork.com/fl/yourname)
- 🟢 [Fiverr · 套餐下单](https://www.fiverr.com/yourname)
- 📧 [Email · 预约演示](mailto:you@example.com?subject=Portfolio%20Showcase%20Booking)
